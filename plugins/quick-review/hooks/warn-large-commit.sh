...
if [[ "$diff_lines" -gt 300 ]]; then
  cat <<EOF
{
  "systemMessage": "⚠️ Large commit: ${diff_lines} lines changed. As you know, it is nice to have small self-contained commits. This message was triggered by the last commit, but it is only a reminder, use your own judgement."
}
EOF
fi
